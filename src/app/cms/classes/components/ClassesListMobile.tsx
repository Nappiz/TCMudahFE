...
import { BookOpen, Package, PencilLine, Trash2, Users2 } from "lucide-react";
...
export function ClassesListMobile({
  ...
  const getPackageItemLabel = (
    ...
    return `${klass?.title || "Unknown"}${offer ? ` • ${offer.meeting_count}x` : ""}`;
  };
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {"curriculum_ids" in it
                  ? it.curriculum_ids.map((id: string) => {
                      const c = idxCurriculum.get(id);
                      return (
                        <span
                          key={id}
                          className="inline-flex items-center gap-1 rounded-lg border border-white/10 bg-white/5 px-2 py-0.5 text-[10px]"
                        >
                  ...
                  : ("class_ids" in it ? it.class_ids : []).map(
                      (cid: string) => (
                        <span
                          key={cid}
                          className="inline-flex items-center gap-1 rounded-lg border border-amber-500/20 bg-amber-500/10 text-amber-200 px-2 py-0.5 text-[10px]"
                        >
