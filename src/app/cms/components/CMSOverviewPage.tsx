...
function Card({
  ...
  return (
    <div
      className={`relative rounded-2xl border border-white/[0.06] bg-gradient-to-b from-[#111827]/90 to-[#0d1117]/90 overflow-hidden ${className}`}
    >
      {/* Top shimmer line */}
...
function KpiCard({
  ...
  const inner = (
    <div className="group relative rounded-2xl border border-white/[0.06] bg-gradient-to-br from-[#111827] to-[#0d1117] p-5 transition-all duration-300 hover:border-white/[0.12] hover:shadow-lg hover:shadow-black/20">
      ...
      <div
        className={`mb-4 flex h-10 w-10 items-center justify-center rounded-xl ${iconColor} transition-transform duration-300 group-hover:scale-105`}
      >
        {icon}
      </div>
  ...
  return (
    <div className="grid gap-4 grid-cols-2 lg:grid-cols-5">
      <KpiCard
        ...
        iconColor="bg-emerald-500/10 ring-1 ring-emerald-500/20"
        title="Revenue"
        value={loading ? "..." : rupiah(period.revenueApproved)}
        subtitle="Dari order approved"
      />
      <KpiCard
        icon={<ShoppingCart className="h-5 w-5 text-blue-400" />}
        iconColor="bg-blue-500/10 ring-1 ring-blue-500/20"
        title="Total Order"
        value={loading ? "..." : period.totalOrders}
        subtitle={`${period.pendingOrders} menunggu review`}
      />
      <KpiCard
        icon={<Users className="h-5 w-5 text-violet-400" />}
        iconColor="bg-violet-500/10 ring-1 ring-violet-500/20"
        title="Total User"
        value={loading ? "..." : stats.totalUsers}
        subtitle={`+${stats.newUsers30d} baru (30h)`}
        href="/cms/user-role"
      />
      <KpiCard
        icon={<Activity className="h-5 w-5 text-cyan-400" />}
        iconColor="bg-cyan-500/10 ring-1 ring-cyan-500/20"
        title="Peserta Aktif"
        value={loading ? "..." : stats.participantsActive}
        subtitle="Sudah bayar & approved"
      />
      <KpiCard
        icon={<GraduationCap className="h-5 w-5 text-amber-400" />}
        iconColor="bg-amber-500/10 ring-1 ring-amber-500/20"
        title="Total Kelas"
        value={loading ? "..." : stats.totalClasses}
        subtitle={`${stats.visibleClasses} published`}
        href="/cms/classes"
      />
    </div>
  );
            ...
            >
              <div
                ...
                style={{
                  ...
                  ["--bar-from" as string]: barColorFrom.includes("cyan")
                    ? "rgb(22 78 99 / 0.5)"
                    : "rgb(30 58 138 / 0.5)",
                  ["--bar-to" as string]: barColorFrom.includes("cyan")
                    ? "rgb(6 182 212 / 0.5)"
                    : "rgb(59 130 246 / 0.5)",
                }}
              />
              ...
              {i % 2 === 0 && (
                <span className="text-[8px] text-slate-600 mt-2 hidden lg:block">
                  {d.label.split(" ")[0]}
                </span>
              )}
            </div>
      ...
      {pendingLatest.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-xs text-slate-600">
            Tidak ada order yang menunggu review.
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-left">
            ...
            <tbody>
              {pendingLatest.map((o, i) => (
                <tr
                  key={o.id}
                  className={`group hover:bg-white/[0.015] transition-colors ${i !== pendingLatest.length - 1 ? "border-b border-white/[0.03]" : ""}`}
                >
...
function OrderStatusBreakdown({ period }: { period: DashboardPeriod }) {
  const total = period.totalOrders || 1;

  const items = [
    {
      label: "Pending",
      count: period.pendingOrders,
      color: "bg-amber-500",
      ...
      <div className="h-2.5 w-full flex rounded-full overflow-hidden bg-slate-800/80 mb-5">
        {items.map((it) => (
          <div
            ...
            style={{
              width: `${Math.max(it.count > 0 ? 2 : 0, (it.count / total) * 100)}%`,
            }}
          />
        ))}
      </div>
        ...
        {items.map((it) => (
          <div key={it.label} className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <span
                className={`text-[13px] font-bold font-mono ${it.textColor}`}
              >
                {it.count}
              </span>
...
function RevenueBreakdown({ period }: { period: DashboardPeriod }) {
  const classRev = period.classRevenue;
  const packageRev = period.packageRevenue;
  const totalRev = classRev + packageRev;
        ...
        <div className="space-y-4">
          ...
          <div className="h-2.5 w-full flex rounded-full overflow-hidden bg-slate-800/80">
            <div
              ...
              className="bg-gradient-to-r from-cyan-600 to-cyan-400 transition-all duration-700"
            />
            <div
              style={{ width: `${packagePct}%` }}
              className="bg-gradient-to-r from-violet-600 to-violet-400 transition-all duration-700"
            />
          </div>
...
function TopClasses({ topClasses }: { topClasses: DashboardTopClass[] }) {
  const medals = ["🥇", "🥈", "🥉"];
  ...
  return (
    ...
    >
      {topClasses.length === 0 ? (
        <p className="p-5 text-[12px] text-slate-600 text-center">
        ...
        <div>
          {topClasses.map((c, i) => (
            <div
              key={c.id}
              className={`flex items-center gap-4 px-5 py-3.5 hover:bg-white/[0.015] transition-colors ${i !== topClasses.length - 1 ? "border-b border-white/[0.03]" : ""}`}
            >
...
function BusinessMetrics({ period }: { period: DashboardPeriod }) {
  const metrics = [
    {
      label: "Avg Order Value",
      value: rupiah(period.aov),
      icon: <DollarSign className="w-3.5 h-3.5 text-emerald-400" />,
      color: "bg-emerald-500/[0.06] border-emerald-500/10",
    },
    {
      label: "Approval Rate",
      value: `${period.approvalRate}%`,
      icon: <CheckCircle className="w-3.5 h-3.5 text-blue-400" />,
      color: "bg-blue-500/[0.06] border-blue-500/10",
    },
  ];
    ...
    <Card title="Business Metrics" icon={<TrendingUp className="w-4 h-4" />}>
      <div className="space-y-2.5">
        {metrics.map((m) => (
          <div
            key={m.label}
            className={`flex items-center justify-between p-3 rounded-xl border ${m.color}`}
          >
            <div className="flex items-center gap-2.5">
              {m.icon}
  ...
  const items = [
    {
      ...
      sub: `${stats.totalCurr} mata kuliah`,
      color: "bg-cyan-500",
    },
    {
      label: "Kelas",
      value: stats.totalClasses,
      sub: `${stats.visibleClasses} publik • ${stats.totalClasses - stats.visibleClasses} draft`,
      color: "bg-blue-500",
    },
    {
      label: "Mentor",
      value: stats.totalMentors,
      sub: `${stats.visibleMentors} publik`,
      color: "bg-violet-500",
    },
    {
      label: "Testimoni",
      value: stats.totalT,
      sub: `${stats.visibleT} publik • ${stats.hiddenT} draft`,
      color: "bg-amber-500",
    },
  ];
        ...
        {items.map((it) => (
          <div key={it.label} className="space-y-1.5">
            ...
            <div className="h-1.5 w-full rounded-full bg-slate-800/80 overflow-hidden">
              <div
                className={`h-full rounded-full ${it.color}/60 transition-all duration-700`}
    ...
    <Card title="Distribusi User" icon={<Users className="w-4 h-4" />}>
      ...
      <div className="h-2.5 w-full flex rounded-full overflow-hidden bg-slate-800/80 mb-5">
        {roles.map((r) => (
          <div
            key={r.label}
            className={`${r.dot} transition-all duration-700`}
            style={{
              width: `${Math.max(r.count > 0 ? 2 : 0, (r.count / total) * 100)}%`,
            }}
          />
        ))}
      </div>
        ...
        {roles.map((r) => (
          <div key={r.label} className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-slate-600 font-mono w-9 text-right">
      ...
      {activities.length === 0 ? (
        ...
        <div className="max-h-[380px] overflow-y-auto">
          {activities.map((act, i) => (
            <div
              key={act.id}
              className={`flex items-start gap-3 px-5 py-3 hover:bg-white/[0.015] transition-colors ${i !== activities.length - 1 ? "border-b border-white/[0.03]" : ""}`}
            >
              <div
                className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full ring-1 ${act.color}`}
              >
                {act.icon}
              </div>
              <div className="min-w-0 flex-1">
              ...
              <span className="text-[10px] text-slate-700 whitespace-nowrap shrink-0">
                {timeAgo(act.time)}
              </span>
            </div>
          ))}
...
function QuickLinks() {
  const links = [
    ...
    {
      href: "/cms/enrollments",
      label: "Atur Enrollment",
    ...
    <Card title="Akses Cepat" icon={<Zap className="w-4 h-4" />}>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
        {links.map((l) => (
          ...
          >
            <span className={`transition-all duration-200 ${l.color}`}>
              {l.icon}
            </span>
...
export default function CMSOverviewPage() {
  ...
  if (loading && !stats) {
    return (
      <div className="h-[70vh] flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-[12px] text-slate-600 animate-pulse">
          Memuat dashboard...
        </p>
      </div>
    );
  ...
  if (!stats || !period)
    return (
      <div className="text-center p-10 text-rose-400">
        Gagal memuat data statistik.
      </div>
    );

  const totalNotif =
    notifData.new_orders + notifData.new_users + notifData.new_feedbacks;
  ...
  return (
    <div className="space-y-6 pb-12">
          ...
          <div className="relative flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="mt-1 text-[12px] text-cyan-200/80 leading-relaxed">
                Terdapat
                {notifData.new_orders > 0 && (
                  <span className="font-semibold text-cyan-400">
                    {" "}
                    {notifData.new_orders} pesanan baru
                  </span>
                )}
                {notifData.new_orders > 0 &&
                  (notifData.new_users > 0 || notifData.new_feedbacks > 0) &&
                  ","}
                {notifData.new_users > 0 && (
                  <span className="font-semibold text-cyan-400">
                    {" "}
                    {notifData.new_users} user baru
                  </span>
                )}
                {notifData.new_users > 0 &&
                  notifData.new_feedbacks > 0 &&
                  " dan"}
                {notifData.new_feedbacks > 0 && (
                  <span className="font-semibold text-cyan-400">
                    {" "}
                    {notifData.new_feedbacks} feedback baru
                  </span>
                )}{" "}
      ...
      {/* KPI Cards */}
      <KpiRow stats={stats} period={period} loading={loading} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <BarChartSection
          ...
          barColorFrom="from-cyan-900/40"
          barColorTo="to-cyan-500/40"
        ...
        <BarChartSection
          ...
          barColorFrom="from-blue-900/40"
          barColorTo="to-blue-500/40"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
        <div className="lg:col-span-2 space-y-5">
          ...
          />
          <TopClasses topClasses={period.topClasses} />
        </div>
        <div className="space-y-5">
          <OrderStatusBreakdown period={period} />
