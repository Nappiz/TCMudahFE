    ...
    <section id="features" className="py-32 relative bg-slate-950 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {features.map((f, i) => (
            <FadeIn key={i} delay={i * 100} className={f.colSpan}>
                <div
                  className={`group relative overflow-hidden rounded-[2rem] bg-slate-900 border border-white/5 p-8 hover:border-white/10 transition-colors h-full`}
                >
                  <div className={`absolute inset-0 bg-gradient-to-br ${f.color} opacity-0 group-hover:opacity-5 transition-opacity duration-500`} />              
                  <div className={`absolute -top-20 -right-20 w-40 h-40 bg-gradient-to-br ${f.color} blur-3xl opacity-20 group-hover:opacity-40 transition-opacity duration-500 hidden sm:block`} />
